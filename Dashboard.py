...
from orders import orderClass #sales
from sales import salesClass #bills
from tkinter import messagebox
from profit import profitClass
import time
import os
import sqlite3
class IMS:
    def __init__(self,root):
        ...
        btn_bills=Button(LeftMenu,text="Bills",command=self.sales,font=("times new roman",15),bg="white",bd=2,cursor="hand2").pack(side=TOP,fill=X)
        btn_orders=Button(LeftMenu,text="Orders",command=self.orders,font=("times new roman",15),bg="white",bd=2,cursor="hand2").pack(side=TOP,fill=X)
        btn_profit=Button(LeftMenu,text="Profit",command=self.profit,font=("times new roman",15),bg="white",bd=2,cursor="hand2").pack(side=TOP,fill=X)
    ...
    def orders(self):   #self to use members of IMS,root
        self.new_win=Toplevel(self.root)
        self.new_obj=orderClass(self.new_win)
    
    def profit(self):   #self to use members of IMS,root
        self.new_win=Toplevel(self.root)
        self.new_obj=profitClass(self.new_win)
